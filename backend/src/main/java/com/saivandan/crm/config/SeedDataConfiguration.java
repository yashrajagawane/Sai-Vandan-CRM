...
class SeedDataConfiguration {
  ...
  private void seedOperationalData(JdbcTemplate jdbc, AppUserRepository users) {
    ...
    UUID support = userId(users,"support@saivandan.local"); jdbc.update("insert into support_tickets(ticket_number,booking_id,category,priority,status,subject,description,assigned_to,due_at) values ('SUP-2026-001',?,'Documentation','HIGH','OPEN','Agreement copy request','Customer requested a signed agreement copy',?," + dateAdd("CURRENT_TIMESTAMP", "DAY", 2, postgres) + ")",booking,support);
