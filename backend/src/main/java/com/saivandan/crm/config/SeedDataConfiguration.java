...
class SeedDataConfiguration {
  ...
  private void seedOperationalData(JdbcTemplate jdbc, AppUserRepository users) {
    ...
    UUID hrUser=userId(users,"hr@saivandan.local"); UUID emp1=jdbc.queryForObject("select id from employees where employee_code='EMP-001'",UUID.class); UUID emp2=jdbc.queryForObject("select id from employees where employee_code='EMP-002'",UUID.class); jdbc.update("insert into attendance_records(employee_id,attendance_date,status,check_in,check_out,overtime_hours,remarks,created_by) values (?,CURRENT_DATE,'PRESENT',CURRENT_TIMESTAMP,DATEADD('HOUR',8,CURRENT_TIMESTAMP),1.5,'Site visit overtime',?)",emp1,hrUser); jdbc.update("insert into attendance_records(employee_id,attendance_date,status,check_in,check_out,remarks,created_by) values (?,CURRENT_DATE,'PRESENT',CURRENT_TIMESTAMP,DATEADD('HOUR',8,CURRENT_TIMESTAMP),'On time',?)",emp2,hrUser); jdbc.update("insert into leave_requests(employee_id,leave_type,start_date,end_date,days,reason,status) values (?, 'CASUAL',DATEADD('DAY',5,CURRENT_DATE),DATEADD('DAY',6,CURRENT_DATE),2,'Family function','PENDING')",emp1);
    ...
    jdbc.update("insert into maintenance_visits(booking_id,ticket_id,scheduled_date,scheduled_time,technician,category,status,notes,created_by) values (?,(select id from support_tickets where ticket_number='SUP-2026-001'),DATEADD('DAY',3,CURRENT_DATE),'10:30 AM','Ramesh Electricals','Electrical','SCHEDULED','Inspect Wing B lift lobby panel',?)",booking,support);
